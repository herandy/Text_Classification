...
def CNN_model(N_HIDDEN=24):
    ...
    drop_emb = Dropout(rate=DROP_OUT)(emb)

    # permuted_emb = Permute((2, 1))(drop_emb)

    conv1 = Conv1D(N_HIDDEN * 3, 3, activation='selu', padding="same", kernel_regularizer=regularizers.l2(REGVAL),
                   kernel_initializer='lecun_normal')(drop_emb)
    pool1 = MaxPooling1D(pool_size=2)(conv1)
    drop1 = AlphaDropout(rate=DROP_OUT)(pool1)

    conv2 = Conv1D((N_HIDDEN * 3) // 2, 3, activation='selu', padding="same",
                   kernel_regularizer=regularizers.l2(REGVAL),
                   kernel_initializer='lecun_normal')(drop1)
    pool2 = MaxPooling1D(pool_size=2)(conv2)
    drop2 = AlphaDropout(rate=DROP_OUT)(pool2)

    conv3 = Conv1D((N_HIDDEN * 3) // 4, 3, activation='selu', padding="same",
                   kernel_regularizer=regularizers.l2(REGVAL),
                   kernel_initializer='lecun_normal')(drop2)
    pool3 = MaxPooling1D(pool_size=2)(conv3)
    drop3 = AlphaDropout(rate=DROP_OUT)(pool3)

    globalpool = GlobalMaxPooling1D()(drop3)

    dense1 = Dense(N_HIDDEN * 3, activation='selu', kernel_initializer='lecun_normal', kernel_regularizer=regularizers.l2(REGVAL))(globalpool)
    drop4 = AlphaDropout(rate=DROP_OUT)(dense1)

    cnn_dense = Dense(NCLASS, activation='softmax', kernel_regularizer=regularizers.l2(REGVAL), name='cnn_dense')(drop4)

    model = Model(inputs=[lin], outputs=cnn_dense)
...
noisy_prediction = K.function(inputs=[model.layers[0].input, K.learning_phase()],
                              outputs=[model.get_layer('cnn_dense').output])
...
# small.fit(data, np_utils.to_categorical(labels), epochs=EPOCHS, batch_size=BATCH_SIZE,
#           validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
# lstm_teacher.fit(data, np_utils.to_categorical(labels), epochs=EPOCHS, batch_size=BATCH_SIZE,
#                  validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#
# EPOCHS = int(EPOCHS / 10)
#
# for teachsess in range(20):
#     print('Teacher is learning...')
#     lstm_teacher.fit(data, np_utils.to_categorical(labels), epochs=EPOCHS, batch_size=BATCH_SIZE,
#                      validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#     print('Teacher prepares study material for students...')
#     lstm_preds = lstm_teacher.predict(data)
#
#     print('Good student is learning...')
#     model.fit(data, lstm_preds, epochs=int(EPOCHS / 2), batch_size=BATCH_SIZE,
#               validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#     model.fit(data, np_utils.to_categorical(labels), epochs=EPOCHS, batch_size=BATCH_SIZE,
#               validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#     cnn_preds = model.predict(data)
#
#     print('Push bad student...')
#     small.fit(data, lstm_preds, epochs=int(EPOCHS / 2), batch_size=BATCH_SIZE,
#               validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#     small.fit(data, np_utils.to_categorical(labels), epochs=EPOCHS, batch_size=BATCH_SIZE,
#               validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#     small_preds = small.predict(data)
#
#     print('Teacher evaluates what they have learned...')
#     lstm_teacher.fit(data, cnn_preds, epochs=int(EPOCHS / 2), batch_size=BATCH_SIZE,
#                      validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#     lstm_teacher.fit(data, small_preds, epochs=int(EPOCHS / 2), batch_size=BATCH_SIZE,
#                      validation_data=(validdata, np_utils.to_categorical(validlabels)), callbacks=[metric])
#
#     test_pred = lstm_teacher.predict(testdata)
#
#     print('test lstm_teacher:')
#     # print(metrics.classification_report(testlabels, np.argmax(test_pred, axis=1)))
#     cm = metrics.confusion_matrix(testlabels, np.argmax(test_pred, axis=1))
#     print('Confusion Matrix: ')
#     print(cm)
#     p, r, f1, s = metrics.precision_recall_fscore_support(testlabels, np.argmax(test_pred, axis=1))
#     print('f1: ', np.average(f1, weights=s))
#
...
#     print('f1: ', np.average(f1, weights=s))
#
#     test_pred = small.predict(testdata)
#
#     print('test small:')
#     # print(metrics.classification_report(testlabels, np.argmax(test_pred, axis=1)))
#     cm = metrics.confusion_matrix(testlabels, np.argmax(test_pred, axis=1))
